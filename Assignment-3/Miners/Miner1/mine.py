...
def handle_miner_transaction(transaction):
    global WALLET_ADDRESS
    WALLET_ADDRESS = transaction["content"].get("To")
    transaction_json=json.dumps(transaction)
    hash_value = computeHash(transaction_json)
    with open(PENDING_DIR+"/"+hash_value+".json", "w") as fp:
        fp.write(transaction_json)
...
async def handle_client(reader, writer):
    ...
    try:
        ...
        if 'id' in message:
            ...
            elif message['id'] == "miner_transaction":
                handle_miner_transaction(message["signed_transaction"])
            else:
...
def writeToNodes(input):
    input_json = json.dumps(input)
    for i in nodes:
            client_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            client_socket.connect(("localhost", nodes[i]))
...
def handle_coinbase_transaction(block_data):
    ...
    flag = True
    while flag:
        transaction_amount = input("Please enter the reward amount of 5: ")
        ...
        if file_count==0:
            ...
            if block_body!=0:
                ...
                with open(ACCOUNT_FILE, 'w') as fp:
                    ...
                    fp.close()
            balances_to_send={"id":"balance","balance":balances}
            writeToNodes(balances_to_send)
